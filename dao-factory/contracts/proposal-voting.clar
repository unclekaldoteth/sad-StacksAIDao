...
(define-public (vote (proposal-id uint) (vote-option uint))
  (let
    ...
    (asserts! (contract-call? .proposal-submission is-proposal-active proposal-id) ERR-PROPOSAL-NOT-ACTIVE)
...
(define-public (conclude (proposal-id uint))
  (let
    (
      (proposal-data (unwrap! (contract-call? .proposal-submission get-proposal proposal-id) ERR-PROPOSAL-NOT-FOUND))
    ...
    (try! (as-contract (contract-call? .proposal-submission set-proposal-status proposal-id new-status)))
